...
// const track_transaction = async (req, res) => {
//   const cookies = req.cookies;
//   if (!cookies?.jwt) return res.sendStatus(401);
//   const refreshToken = cookies.jwt;
//   const current_time = await fetchTime();
//   const foundUser = await User.findOne({ refreshToken }).exec();
//   if (!foundUser) return res.sendStatus(0);
//   const { transaction_id } = req.body;
//   if (!transaction_id) { return res.status(400).json({ message: "transaction p are required." }); }

//   const transaction_found = await FundSchema.findOne({ tx_ref: transaction_id }).exec();
//   if (!transaction_found) { return res.status(400).json({ message: "no record fund for this transaction with id" }); }

//   if (transaction_found.status === 'successful') {
//     return res.status(200).json(transaction_found.status);
//   } else {
